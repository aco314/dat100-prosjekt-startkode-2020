...
public class GPSComputer {
	...
	public int totalTime() {
		...
		int timeFirst = gpspoints[0].getTime();
		int timeLast = gpspoints[gpspoints.length-1].getTime();
	 ...
	 * bicycling, 16-19.9 mph, racing/not drafting or >19 mph drafting, very fast, racing general 12.0
	...
	public double totalKcal(double weight) {
		...
		for (int i = 0; i < gpspoints.length-1; i++) {
			int secDiff = gpspoints[i+1].getTime() - gpspoints[i].getTime();
			totalkcal += kcal(weight, secDiff, speeds[i]);
		}
